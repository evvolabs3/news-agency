...
export async function syncLatestNews(): Promise<void> {
  ...
  if (!apiKey) {
    ...
    if (!hasSyncedOnce) upsertArticles(fallbackArticles);
    hasSyncedOnce = true;
    return;
  }
...
export async function fetchNews(query: NewsQuery): Promise<Article[]> {
  await syncLatestNews();
  return queryArticles(query);
}

export async function fetchAnalysis(articleId: string): Promise<AnalysisResult> {
